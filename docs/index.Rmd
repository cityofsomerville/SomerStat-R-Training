...
# 3. Call read.socrata() without the config argument
happinessSurveyData <- RSocrata::read.socrata(url)
happinessSurveyData$happiness_num <- as.numeric(happinessSurveyData$happiness_num)

only17Younger <- happinessSurveyData %>% dplyr::filter(happinessSurveyData$age == "17 or under") 
...
```{r filterex3-hint-1}
# Check for the dataset id here, I typically find it in the url! <https://data.somervillema.gov/Health-Wellbeing/Somerville-Happiness-Survey-Responses/wmeh-zuz2/about_data>
...
Something I often do once the code will compile and run is assume that I've made every mistake possible, so I like to get an overview of the dataset. For a dataset as large as the happiness survey, it can be tough to just open up excel and take a peek, so I often do this in R. I usually start by taking a look at what columns we have.

```{r filterex4, exercise = TRUE}
happinessSurveyData %>% colnames()
```
...
happinessSurveyData %>% dplyr::filter(is.na(id)) %>% nrow()
```
...
QAdataframe <- happinessSurveyData %>%
    group_by(year) %>%
    summarise(mean = mean(happiness_num, na.rm = TRUE), 
              median = median(happiness_num, na.rm = TRUE), 
              stdev = sd(happiness_num, na.rm = TRUE))
```
...
happinessDataCleaned <- happinessSurveyData %>% filter(!(age == "17 or under" & !is.na(age))) %>% select(-age)
```
